...
func TestBubblewrapArgsAppliesNetworkPolicy(t *testing.T) {
	...
	if err := os.WriteFile(networkConfig, []byte("config"), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestRollbackPatchFilesRestoresOriginalContent(t *testing.T) {
	...
	if err := os.WriteFile(first, []byte("changed first"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(second, []byte("changed second"), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestAtomicWritePreservesMode(t *testing.T) {
	...
	if err := os.WriteFile(path, []byte("old"), 0754); err != nil {
		t.Fatal(err)
	}
	...
	if got := info.Mode().Perm(); got != 0754 {
...
func TestFileAndDirectoryTransfers(t *testing.T) {
	...
	if err := tw.WriteHeader(&tar.Header{Name: "nested/hello.txt", Mode: 0600, Size: int64(len(content)), Typeflag: tar.TypeReg}); err != nil {
		t.Fatal(err)
	}
...
func TestDirectoryUploadRejectsPathTraversal(t *testing.T) {
	...
	if err := tw.WriteHeader(&tar.Header{Name: "../escape.txt", Mode: 0600, Size: int64(len(content)), Typeflag: tar.TypeReg}); err != nil {
		t.Fatal(err)
	}
...
func TestEditRejectsOversizedFile(t *testing.T) {
	...
	if err := os.WriteFile(path, bytes.Repeat([]byte("a"), 100), 0600); err != nil {
		t.Fatal(err)
	}
...
func TestApplyPatchCommitsWithoutGlobalLock(t *testing.T) {
	...
	if err := os.WriteFile(first, []byte("aaa"), 0600); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(second, []byte("bbb"), 0600); err != nil {
		t.Fatal(err)
	}
	result, err := server.applyPatch(json.RawMessage(`{
		"edits": [
